...
const brandId = Cookie.get("kry-api-brand-id");
...
/* global __DEV__:false __MOCK__:false __HOST_API__:false */
const handleUrl = (url, requestSet) => {
  if (
    (__DEV__ && __MOCK__) ||
...
//send
const send = (method, url, params, requestSet) => {
...
//GET
const get = (url, params, requestSet) => {
  return send("get", url, params, requestSet);
};

//POST
const post = (url, params, requestSet) => {
  return send("post", url, params, requestSet);
};

// Form
const postForm = (url, params, requestSet) => {
